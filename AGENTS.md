...
│   ├── SparkArt.tsx         Mono line illustrations per Spark (accent-colored), replaces emoji tiles everywhere
...
└── lib/
    ...
    ├── agent.ts             Anthropic loop + 7-tool toolbelt (get_current_draft, check_ens_subname, draft_dapp_manifest, list_sparks, get_capabilities, resolve_ens_name, suggest_labels) + template fallback
    ├── homeShortcuts.ts     localStorage order of Home "Sparks" tiles (editable: add/remove/rearrange)
